...
from retrieve_env import search_mail, execute_sql
...
# Role Description
    ...
    - `contact_email`         TEXT     -- 联系人唯一 ID（主键）以及联系人邮箱地址（非结构化，需要使用向量检索工具，返回符合语义的 person_id 列表）
    - `contact_name`      TEXT     -- 联系人姓名（非结构化，需要使用向量检索工具，返回符合语义的 person_id 列表）
    - `email_count`       INTEGER  -- 与该联系人的往来邮件数量（结构化，可用于 SQL 条件过滤）
    - `relationship`      TEXT     -- 与该联系人的关系（如"同事"、"家人"等）（非结构化，需使用向量检索工具，返回符合语义的 person_id 列表）
...
PROMPT = """
你是资深 AI 文件检索工程师，负责将自然语言查询转为带 <vector_x> 占位符的 SQL 查询，并生成对应向量参数列表。

要求：
- 查询仅限 email 表或 person 表，不能跨表。
- 非结构化字段必须用向量匹配（不可用 LIKE），匹配结果通过 ID 逻辑连接（IN）。
- 返回字段只能是：message_id / folder_id / attachment_id / person_id。
- 向量占位符依次为 <vector_0> ~ <vector_9>，按 SQL 中字段出现顺序分配。
- JSON 字段用 json_each 与 json_extract 配合。
- 时间模糊词如“最近”用 date('now', '-7 day') 等表示,今天是2025年7月25日,时间要注意相对时间的变化。

字段列表：

【email 表】
- 结构化字段（可直接 SQL 匹配）：
  account_email, received_date, is_draft, draft_created_date, draft_modified_date, is_read, is_starred, is_archived, thread_message_count

- 非结构化字段（需向量匹配）：
  sender_email, sender_name, recipient_list, cc_list, bcc_list, subject, email_content, folder_labels, attachment_list
...
- 非结构化字段：
  person_id（邮箱地址）, contact_name, relationship
...
if __name__ == "__main__":
    ...
    parser.add_argument("--local_dir", default="/workspace/yunhai/data/mail_ai_simple")
    parser.add_argument("--hdfs_dir", default=None)
    args = parser.parse_args()

    data_source = "/workspace/yunhai/rl-date-gen/sql_and_query_v5_email_with_sql2.json"

    # ① 加载数据
    dataset = load_dataset("json", data_files=data_source)["train"]
    print(f"Loaded {len(dataset)} examples from {data_source}")
    # ② 第一次 map：添加 ground_truth
    def add_ground_truth(example, idx):
        question_raw = example["query"]
        raw_vql = example.get("vector_query_list")
        if raw_vql is None:
            raw_vql = []
        ...
        for cond in raw_vql:
            ...
            if isinstance(cond, dict):
                pruned = {k: v for k, v in cond.items() if v is not None}
                if pruned:                         # 避免 {'x': None}
                    clean_vql.append(pruned)
            else:
                clean_vql.append(cond)             # 若是字符串/数字等其他类型

        result = {
            "sql": example["sql"],
            "vector_query_list": clean_vql,        # 这里总是 list，可能为空 []
        }
        print(result)
        json_str = json.dumps(result, ensure_ascii=False)
        gt = execute_sql(json_str)  # 只调用一次
        if gt is None:
            print(f"查询失败，无法生成 ground_truth: {question_raw}")
            gt = None
        elif len(gt['result']) == 0:
            print(f"查询结果为空，无法生成 ground_truth: {question_raw}")
            gt = None
        else:
            gt = gt['result'] 
            print(f"查询成功，生成 ground_truth: {question_raw} -> {gt[0]}")
        example["ground_truth"] = gt
        example["index"] = idx
        example["sql_ground_truth"] = json_str
        sleep(0.1)  # 避免请求过快
        return example

    dataset = dataset.select(range(3000))
    dataset = dataset.map(add_ground_truth, with_indices=True)

    # ③ 过滤 ground_truth 为空的样本
    dataset = dataset.filter(lambda x: x["ground_truth"] is not None, desc="Drop examples without ground‑truth")
    ...
    def format_for_training(example):
        return {
            ...
            "ability": "math",
            "reward_model": {"style": "rule", "ground_truth": example["ground_truth"], "sql_ground_truth": example["sql_ground_truth"]},
            "extra_info": {
                "split": example.get("split", ""),
                "index": example["index"],
                "answer": example["sql_ground_truth"],
                "question": example["query"],
            },
        }

    train_dataset = train_dataset.map(format_for_training)
    test_dataset = test_dataset.map(format_for_training)
